...
class ServerDataBase:
    ...
    def all_users_list(self):
        lst = self.session.query(self.AllUsers.name, self.AllUsers.last_login)
        return lst.all()
    ...
    def add_contact(self, user, contact):
        ...
        if not contact or self.session.query(self.UserContacts).filter_by(name=user.id, contact=contact.id).count():
            return
